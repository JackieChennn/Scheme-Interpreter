...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = rest.map(lambda expr: scheme_eval(expr, env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        for formal, value in zip(pair_to_list(formals), pair_to_list(vals)):
            local_frame.define(formal, value)
        return local_frame
        # END PROBLEM 10


def pair_to_list(pair):
    """Turn a scheme PAIR into a python list."""
    if pair is nil:
        return []
    else:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        copy_args = args
        if copy_args is not nil:
            python_args.append(copy_args.first)
            while copy_args.rest is not nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambda_function_name = target.first
        lambda_function_formals = target.rest
        lambda_function_body = expressions.rest
        lambda_function = do_lambda_form(
            Pair(lambda_function_formals, lambda_function_body), env)
        env.define(lambda_function_name, lambda_function)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if is_true_primitive(first_value):
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    binding_list = pair_to_list(bindings)
    for binding_pair in binding_list:
        validate_form(binding_pair, 2, 2)
        name, value = binding_pair.first, binding_pair.rest.first
